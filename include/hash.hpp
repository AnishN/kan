...
namespace hash {
    static const uint64_t crc64_lut[256] = {
    ...
    template<typename T>
    uint64_t crc64(T *data, size_t count = 1) {
        uint8_t *ptr = data;
        uint64_t crc = 0;
        for(size_t i = 0; i < count * sizeof(T); ++i) {
            uint8_t byte = data[i];
            crc = crc64_lut[(uint8_t)crc ^ byte] ^ (crc >> 8);
        }
        return crc;
    }

    template<typename T>
    uint64_t crc64_stream(T *data, uint64_t prev, size_t count = 1) {
        uint8_t *ptr = data;
        uint64_t crc = prev;
        for(size_t i = 0; i < count * sizeof(T); ++i) {
            uint8_t byte = data[i];
